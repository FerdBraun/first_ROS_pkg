...
class AStarPlanner(Node):
    def __init__(self):
        ...
        self.last_path = None
        self.min_path_length = 0.3  # Минимальная длина пути в метрах
        self.buffer_distance = 0.3  # Буферная зона в метрах
        self.replan_interval = 5.0  # Интервал перепланирования
        self.min_replan_interval = 5.0  # Минимальный интервал между перепланировками
        self.last_replan_time = time.time()
        self.is_active = True  # Флаг активности планировщика
    ...
    def stop_callback(self, msg):
        ...
        self.status_pub.publish(String(data=f"Planner {status}"))
        self.get_logger().warn(f"Planner received stop command. New state: {status}")
    ...
    def goal_callback(self, msg):
        ...
        if not self.is_active:
            self.get_logger().warn("Planner is stopped, ignoring goal")
            return
    ...
    def execute_planning(self, msg):
        try:
            ...
            distance = math.hypot(map_goal.point.x - map_start.point.x,
                                map_goal.point.y - map_start.point.y)
            if distance < self.min_path_length:
                self.get_logger().info(f"Goal too close ({distance:.2f}m < {self.min_path_length}m), ignoring")
                # Сбрасываем текущую цель
                self.current_goal = None
                # Не публиковать пустой путь, если предыдущий путь уже был пустым
            ...
            if path:
                ...
                self.get_logger().info(f"Path found with length: {path_length:.2f}m")
    ...
    def get_robot_position(self):
        ...
        try:
            ...
            self.get_logger().error(f'Error getting robot position: {str(e)}')
            raise
    ...
    def replan_callback(self):
        ...
        if not self.is_active or self.current_goal is None:
            return
